...
!$ use omp_lib
  ...
  real(kind = 8), dimension(1:16) :: error, start, finish, t_av
...
!$omp single
  ...
  !write(*,*) 'int2 before division', int2,nr
  int2=int2/dble(nr)
  !write(*,*) 'int2 after division', int2,nr
  ...
  !write(*,*) 'int4 before division', int4,nr
  int4=int4/dble(nr)
  !write(*,*) 'int4 after division', int4,nr
  int4=int4+int2
  !write(*,*) 'final int4', int4,nr

  error(tid) = int4**0.5d0  
  ...
  !record time
  if (k == 20) then
    if (nthreads == 0) then !serial code
      open(1,file=trim('s_time20.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') t_av(i)
      end do
    else
      open(1,file=trim('time20.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') t_av(i)
      end do
    end if
    ...
    if (nthreads == 0) then !serial code
      open(1,file=trim('s_time800.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') t_av(i)
      end do
    else
      open(1,file=trim('time800.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') t_av(i)
      end do
    end if
  ...
  if (k == 20) then
    if (nthreads == 0) then !serial code
      open(1,file=trim('s_error20.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') error(i)
      end do
    else
      open(1,file=trim('error20.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') error(i)
      end do
    end if
    ...
    if (nthreads == 0) then !serial code
      open(1,file=trim('s_error800.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') error(i)
      end do
    else
      open(1,file=trim('error800.txt'),status='unknown')
      do i=1,tid
        write(1,fmt='(E24.16)',advance='no') error(i)
      end do
    end if
