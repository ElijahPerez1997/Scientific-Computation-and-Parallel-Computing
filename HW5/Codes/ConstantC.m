...
b_primek_x=@(t,b) gamma2*(xdif(1,round(t*10)+1)-b(k))+ gamma3*((sum(xdif(1,round(t*10)+1)+b(2:n))/n) - b(k)) + p*sum(sum(((b(k)-(xdif(1,round(t*10)+1)+b(2:L))/((b(k)-(xdif(1,round(t*10)+1)+b(2:L))).^2 +delta)))));
...
b_primek_y=@(t,b) gamma2*(xdif(2,round(t*10)+1)-b(k))+ gamma3*((sum(xdif(2,round(t*10)+1)+b(2:n))/n) - b(k)) + p*sum(sum(((b(k)-(xdif(2,round(t*10)+1)+b(2:L))/((b(k)-(xdif(2,round(t*10)+1)+b(1:L))).^2 +delta)))));
...
% Initialize video
myVideo = VideoWriter('ConstantC'); %open video file
